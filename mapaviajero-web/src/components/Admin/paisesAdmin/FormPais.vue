    ...
    <div class="max-w-xl mx-auto p-4">
      ...
      <form @submit.prevent>
        ...
        <div class="mb-4">
          ...
          <select v-model="form.continente_id" class="w-full border px-3 py-2 rounded" :class="{ 'border-red-500': v.continente_id.hayError }">
        ...
        <div class="mb-4">
          ...
          <select v-model="form.conduccion" class="w-full border px-3 py-2 rounded" :class="{ 'border-red-500': v.conduccion.hayError }">
  ...
  methods: {
    validar(){
      ...
      if(this.form.bandera_url){
        const urlValida = /^https?:\/\/.+\.(jpg|jpeg|png|gif|webp|svg)(\?.*)?$/i.test(this.form.bandera_url.trim())
        if (!urlValida) {
          this.v.bandera_url.hayError = true
          this.v.bandera_url.mensajesError = ['Introduce una URL válida de imagen (jpg, png, gif, etc.)']
        }
      }
    },
    async handleSubmit() {
