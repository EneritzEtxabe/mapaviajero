...
export default defineComponent({
  ...
  computed: {
    ...
    ...mapState(useTipoLugaresStore, {
      loadingTipoLugares: 'loading',
      errorTipoLugares: 'error',
      tiposLugar: 'items',
    }),
    ...
    loading() {
      return this.loadingLugar || this.loadingTipoLugares || this.loadingPaises
    },
    error() {
      return this.errorLugar || this.errorTipoLugares || this.errorPaises
    },
    editMode() {
      return !!this.id
    },
    ...
    validar() {
      ...
      if (this.form.imagen_url) {
        const urlValida = /^https?:\/\/.+\.(jpg|jpeg|png|gif|webp|svg)(\?.*)?$/i.test(
          this.form.imagen_url.trim(),
        )
        if (!urlValida) {
          this.v.imagen_url.hayError = true
          this.v.imagen_url.mensajesError = [
      ...
      if (this.form.localizacion_url) {
        const urlValida = /^https:\/\/www\.google\.com\/maps\/embed\?pb=/.test(
          this.form.localizacion_url.trim(),
        )
        if (!urlValida) {
          this.v.localizacion_url.hayError = true
          this.v.localizacion_url.mensajesError = [
            'Introduce una URL válida para un mapa Embebido de Google Maps',
          ]
        }
      }
