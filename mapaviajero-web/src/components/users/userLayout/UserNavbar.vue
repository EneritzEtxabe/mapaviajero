<template>
  <nav
    class="fixed top-0 left-0 right-0 z-50 bg--white f-color--dark px-20 h-16 flex items-center justify-between"
  >
    ...
    <ul class="hidden md:flex space-x-6 items-center">
      <li>
        <router-link
          to="/"
          class="text-md inline-flex items-center justify-center h-9 px-3 hover:underline"
          >Inicio</router-link
        >
      </li>
      <li>
        <router-link
          :to="{ name: 'paises' }"
          class="inline-flex items-center justify-center h-9 px-3 hover:underline"
          >Países</router-link
        >
      </li>
      <li v-if="isAuthenticated">
        <router-link
          :to="{ name: 'alquilerCoches' }"
          class="text-md inline-flex items-center justify-center h-9 px-3 hover:underline"
          >Alquiler de coches</router-link
        >
      </li>
      ...
      <li v-if="!isAuthenticated">
        <router-link
          :to="{ name: 'login' }"
          class="text-md inline-flex items-center justify-center h-9 px-3 pl-20 hover:underline"
          @click="clearError"
          >Login</router-link
        >
      </li>
    ...
    <div
      v-show="isVisible"
      class="absolute left-0 right-0 top-full z-50 bg--white lg:hidden px-4 pb-4"
    >
